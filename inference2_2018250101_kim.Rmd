---
title: "Chapter 15. Statistical Inference (2)"
output:
  pdf_document:
...
Confidence intervals are a very useful concept widely employed by data analysts. A version of these that are commonly seen come from the ```ggplot``` geometry ```geom_smooth```. The shaded area around the curve is created using the concept of confidence intervals.

When an interval is big, we will be ridiculed for stating the obvious. But The smaller the interval we the smaller our chances are of winning the prize. 
...
```{r}
se_hat <- sqrt(x_hat * (1 - x_hat) / N)
clinton <- c(x_hat - 1.96 * se_hat, x_hat + 1.96 * se_hat)
```

2. Now use ```dplyr``` to add a confidence interval as two cloumns, call them ```lower``` and ```upper```, to the object ```poll```. Then use ```select``` to show the ```pollster```, ```enddate```, ```x_hat```, ```lower```, ```upper``` variables.

Answer:
```{r}
library(dplyr)

poll <- polls %>% mutate(x_hat = polls$rawpoll_clinton/100, se_hat = sqrt(x_hat * (1 - x_hat) / N),  lower = x_hat - 1.96 * se_hat, upper = x_hat + 1.96 * se_hat) %>% select(pollster, enddate, x_hat, lower, upper)
```

3. The final tally for the popular vote was Clinton 48.2% and Trump 46.1%. Add a column, call it ```hit```, to the previous stable stating if the confidence interval included the true proportion ```p = 0.482``` or not.
...
The odds ratio is $\frac{a/c}{b/d}$ which is equivalent to $(ad) / (bc)$
...
This is a slightly different p-value than that with the Chi-square test. This is because we are using a different asymptotic approximation to the null distribution. To learn more about inference and asymptotic theory for odds ratio, consult the *Generalized Linear Models* book by
...
Answer:
```{r}
two_by_two <- data.frame(result = c("win", "lose"),
                         career = c(350, 150), impo = c(8, 9))
two_by_two
```
...
Answer: **C. Because the sum of the rows and columns of the two-by-two table are not fixed so the hypergeometric distribution is not an appropriate assumption for the null hypothesis. For this reason, Fisher’s exact test is rarely applicable with observational data.**

3. 

Answer:
```{r}
odds_career <- with(two_by_two, (career[2]/sum(career)) / (career[1]/sum(career)))
odds_impo <- with(two_by_two, (impo[2]/sum(impo)) / (impo[1]/sum(impo)))
```

```{r}
log_or <- log(odds_impo / odds_career)

se <- two_by_two %>% select(-result) %>%
  summarize(se = sqrt(sum(1/impo) + sum(1/career))) %>%
  pull(se)
ci <- log_or + c(-1,1) * qnorm(0.975) * se
exp(ci)
```
...
Answer : **C. Different approximations are used for the p-value and the confidence interval calculation. If we had a larger sample size the match would be better.**

5. Multiply the two-by-two table by 2 and see if the p-value and confidence retrieval are a better match.

Answer:
```{r}
two_by_two %>% select(-result) %>% mutate(impo = impo * 2, career = career * 2) %>% chisq.test() %>% .$p.value
```
