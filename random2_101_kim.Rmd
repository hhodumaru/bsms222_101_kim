---
title: "Chapter 14. Random variables(2)"
output:
  pdf_document:
...
The Central Limit Theorem (CLT) tells us when the number of draws, also called the *sample size*, is large, the probability distribution of the sum of the independent draws is approximately normal. Because sampling models are used for so many data generation processes, the CLT is considered one of the most important mathematical insights in history.
...
The same applies to probability distributions. If a random variable has a probability distribution that is approximated with the normal distribution, we just need to describe the average and standard deviation, referred to as the **expected value and standard error**.
...
3. The square of the standard error of the sum of **independent** random variables is the sum of the square of the standard error of each random variable. The square of the standard error is referred to as the variance in statistical textbooks. 
...
The standard error of the average becomes smaller as $n$ grows larger. When $n$ is very large, the standard error is practically 0 and the average of the draws converges to the average of the urn. 
...
Answer:
```{r}
sqrt(n)*abs((17-(-1)))*sqrt(1/19 * 18/19)
```
...
Answer:
```{r}
mu <- n*(1/19*17 + 18/19*(-1))
se <-  sqrt(n)*abs((17-(-1)))*sqrt(1/19 * 18/19)
1 - pnorm(0, mu, se)
```
...
Answer : **B. The CLT does not work as well when the probability of success is small. In this case, it was 1/19. If we make the number of roulette plays bigger, they will match better.**
...
15. What is the probability that you end up with winnings per game that are positive? 
...
Answer: **c. The CLT works better when the sample size is larger. We increased from 1,000 to 2,500.**
...
The trick works because we end up with an expression containing $x$ that we know has to be equal to a known quantity $z$. 
...
$SE[(X_1+X_2+⋯+X_n)/n]=σ/√n = SE[_nX_1/n] = σ > σ/√n$
...
Answer : **D. The number of Monte Carlo simulations.**