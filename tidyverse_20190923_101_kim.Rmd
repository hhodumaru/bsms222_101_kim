...
My answer
```{r}
ref <- NHANES %>% filter(AgeDecade == " 20-29" && Gender == "female") %>% summarize(mean_BP = mean(BPSysAve, na.rm = TRUE), dev_BP = sd(BPSysAve, na.rm = TRUE))

head(ref)
```

I have no idea why the row information shows ```NaN```.
...
My answer
```{r}
ref_fe <- filter(NHANES, Gender == "female") %>% group_by(AgeDecade) %>% summarize(mean_fe = mean(BPSysAve, na.rm = TRUE), dev_fe = sd(BPSysAve, na.rm = TRUE))
```
...
My answer
```{r}
ref_ma <- filter(NHANES, Gender == "male") %>% group_by(AgeDecade) %>% summarize(mean_ma = mean(BPSysAve, na.rm = TRUE), dev_ma = sd(BPSysAve, na.rm = TRUE))
```
